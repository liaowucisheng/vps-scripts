...
# ============================================================
echo "============================================"
echo " Xray + REALITY 安装脚本 (Docker 版)"
...
# 5. 写入配置
# ============================================================
info "写入配置文件 /etc/xray-docker/config.json ..."
mkdir -p /etc/xray-docker

cat > /etc/xray-docker/config.json <<EOF
{
  "log": {
    "loglevel": "warning"
  },
...
if systemctl is-active --quiet xray 2>/dev/null; then
    warn "检测到宿主机已安装 Xray (systemd 服务)，与 Docker 版端口冲突"
    read -p "是否停止并禁用系统 Xray 服务? (y/n, 默认 n): " STOP_NATIVE
...
docker run -d \
    ...
    --network host \
    -v /etc/xray-docker/config.json:/etc/xray/config.json:ro \
    ghcr.io/xtls/xray-core:latest \
    run -c /etc/xray/config.json >/dev/null 2>&1 || true

sleep 2

# 验证容器
if docker ps --format '{{.Names}}' | grep -qx "xray-reality"; then
    ok "Xray Docker 容器运行中"
else
    err "Xray 容器启动失败！查看日志: docker logs xray-reality"
    exit 1
fi
...
echo ""
echo "============================================================"
highlight "✅ Xray + REALITY (Docker) 安装完成！"
echo "============================================================"
echo ""

highlight "📋 服务器参数"
echo "  协议:       VLESS + REALITY + Vision"
echo "  部署方式:   Docker ($(docker images --format '{{.Repository}}:{{.Tag}}' | grep xray-core))"
echo "  地址:       $SERVER_IP"
echo "  端口:       $PORT"
echo "  UUID:       $UUID"
...
echo ""
highlight "🛠 常用管理命令"
echo "  查看状态:       docker ps -a --filter name=xray-reality"
echo "  查看日志:       docker logs xray-reality"
echo "  实时日志:       docker logs -f xray-reality"
echo "  重启:           docker restart xray-reality"
echo "  停止:           docker stop xray-reality"
echo "  启动:           docker start xray-reality"
echo "  删除:           docker rm -f xray-reality"
echo "  升级:           docker pull ghcr.io/xtls/xray-core:latest && docker restart xray-reality"
echo ""
